...
export function startWorkers() {
  new Worker("quality-checks", async job => {
    ...
    const result = await pool.query("SELECT rows_json FROM datasets WHERE id=$1 AND organization_id=$2", [datasetId, organizationId]);
    if (!result.rowCount) return { skipped:true };
    const rows = result.rows[0].rows_json as Record<string,unknown>[];
    let all=0, missing=0; rows.forEach(row=>Object.values(row).forEach(value=>{all++;if(value===null||value===undefined||String(value).trim()==="")missing++;}));
    await pool.query("INSERT INTO audit_events(organization_id,event_type,entity_type,entity_id,metadata) VALUES($1,'quality.checked','dataset',$2,$3)", [organizationId,datasetId,JSON.stringify({records:rows.length,completeness:all?Math.round((1-missing/all)*100):0})]);
    return { records:rows.length, completeness:all?Math.round((1-missing/all)*100):0 };
  }, { connection });
}
