...
export function ChatInterface() {
  ...
  const ask = async (question: string) => {
    ...
    try {
      ...
      if (manualFilter) plan.filters = [...(plan.filters || []), manualFilter];
      ...
      const filterEvidence = plan.filters?.length ? plan.filters.map((filter: ManualFilter) => filter.field + " = " + filter.value).join(", ") : "none";
      add({ id: crypto.randomUUID(), role: "assistant", content: executiveAnswer(output, plan.field) + "\n\n_Provenance: dataset version " + result.datasetVersion + "; metric: " + plan.field + "; filters: " + filterEvidence + ".", ...chart, insights: result.insights });
    } catch (error) {
